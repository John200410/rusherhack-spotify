...
public class SpotifyHudElement extends ResizeableHudElement {
	...
	public void tick() {
		...
		if(this.updateTimer.passed(1000)) {
			api.updateStatus();
	...
	public void renderContent(RenderContext context, int mouseX, int mouseY) {
		...
		final PlaybackState data = status;
		final PlaybackState.Item song = data.item;
		...
		final double progress = (double) data.progress_ms / (double) song.duration_ms;
		...
		//duration
		final String current = String.format("%d:%02d", data.progress_ms / 60000, data.progress_ms / 1000 % 60);
		...
		//shuffle
		final VectorGraphic shuffleGraphic = data.shuffle_state ? this.shuffleOnGraphic : this.shuffleOffGraphic;
